...
class Color(PrivateInit):
    ...
    def RGB(cls, r: float, g: float, b: float, a:float=1):
        ...
        vals += [a]
        c.color_value = f"rgba({','.join([str(v) for v in vals])})"
        c.rgba = vals
        return c
    ...
    def AsHex(self):
        return '#{:02x}{:02x}{:02x}'.format(*self.rgba[:-1])
...
XColor = str | Color | COLORS
ListOfXColor = list[str]|list[Color]|list[COLORS]|list[XColor]

def ColorObj(col: XColor):
    if isinstance(col, str): return Color.Hex(col)
    if isinstance(col, COLORS): return Color.Hex(col.value)
    return col
